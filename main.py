...
async def process_paper(arxiv_url: str):
    """
    ...
    full_md_path, images_dir = mineru_client.download_and_extract_zip(zip_url, paper_cache_dir)
    print()
    
    # Step 4: Generate summaries with Claude
    print("Step 4: Generating summaries with Claude Agent SDK")
    print("-" * 70)
    ...
    print()
    print(f"Cached MinerU parsed content: {paper_cache_dir}")
    print()
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print()
        print("Example:")
        print("  python main.py https://arxiv.org/pdf/1706.03762.pdf")
        sys.exit(1)
    
    arxiv_url = sys.argv[1]
    
    try:
        asyncio.run(process_paper(arxiv_url))
